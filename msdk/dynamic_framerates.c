...
//gcc -g -Wall -pthread `pkg-config --cflags glib-2.0` `pkg-config --cflags gstreamer-video-1.0` dynamic_framerates.c -o dynamic_framerates `pkg-config --libs glib-2.0` `pkg-config --libs gstreamer-video-1.0`

static gchar* caps[] = {
    "video/x-raw,format=YUY2, width=1280,height=720,framerate=30/1",
    "video/x-raw,format=YUY2, width=1280,height=720,framerate=40/1",
...
static gboolean
change_framerate(GstElement *filter) {
    GstCaps *srccaps;
...
{
    GstElement *pipeline, *src, *filter, *enc, *dec, *sink, *subsink;
    GstCaps *srccaps;
    GMainLoop *loop;
    GstBus *bus;
    guint bus_watch_id;
    ...
    subsink = gst_element_factory_make("glimagesink", "subsink");

    if (!pipeline || !src || !filter || !enc || !dec || !sink || !subsink) {
        ...
        GST_OBJECT_FREE_AND_LOG(subsink);
        g_printerr("Exiting...\n");
    ...
    g_object_set(src, "num-buffers", 1200, NULL);

    srccaps = gst_caps_from_string("video/x-raw,format=YUY2, width=1280,height=720,framerate=25/1");
    g_object_set(filter, "caps", srccaps, NULL);
    gst_caps_unref(srccaps);

    g_object_set(sink, "video-sink", subsink, NULL);
    ...
    /* Change frame rate in call back */
    g_timeout_add(4000, (GSourceFunc) change_framerate, filter);
